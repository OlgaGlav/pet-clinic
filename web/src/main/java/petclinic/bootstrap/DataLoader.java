...
public class DataLoader implements CommandLineRunner {
    ...
    private void loadData() {
        ...
        owner1.setAddress("123 Briston");
        owner1.setCity("Maiami");
        owner1.setTelephone("1254568");

        Pet fionasCet = new Pet();
        fionasCet.setPetType(savedCatPetType);
        fionasCet.setOwner(owner2);
        fionasCet.setBirthDate(LocalDate.now());
        fionasCet.setName("Just Cat");
        owner2.getPets().add(fionasCet);

        ownerService.save(owner2);

        Visit catVisit = new Visit();
        catVisit.setPet(fionasCet);
        catVisit.setDate(LocalDate.now());
        catVisit.setDescription("Sneezzy Kety");
